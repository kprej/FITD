...
const unsigned char defaultPalette[0x30] = {
    0x00, 0x00, 0x00, 0x3F, 0x3F, 0x3F, 0x0C, 0x0C, 0x0E, 0x30, 0x2F, 0x3F,
    0x23, 0x2C, 0x23, 0x2A, 0x1D, 0x2A, 0x2A, 0x21, 0x18, 0x3F, 0x05, 0x2A,
    0x12, 0x14, 0x18, 0x31, 0x15, 0x17, 0x15, 0x25, 0x15, 0x15, 0x2F, 0x3F,
    0x3F, 0x22, 0x15, 0x2B, 0x15, 0x3F, 0x3F, 0x3F, 0x21, 0x3F, 0x3F, 0x3F};

const unsigned char defaultPaletteAITD3[0x30] = {
    0x00, 0x00, 0x00, 0xFC, 0xFC, 0xFC, 0x30, 0x30, 0x38, 0xC0, 0xBC, 0xFC,
    0x78, 0x58, 0x3C, 0x00, 0x00, 0x00, 0xF0, 0x70, 0x10, 0xFC, 0xFC, 0xFC,
    0x48, 0x50, 0x60, 0xC4, 0x54, 0x5C, 0x54, 0x94, 0x54, 0x54, 0xBC, 0xFC,
    0xFC, 0x88, 0x54, 0xAC, 0x54, 0xFC, 0xFC, 0xFC, 0xFC, 0xFC, 0xFC, 0xF8};
...
void executeFoundLife (int objIdx)
{
    ...
    if (actorIdx == -1)
    {
        ...
        if (currentActorEntry ==
            -1) // no space, we will have to overwrite the last actor !
        {
...
void allocTextes (void)
{
    ...
    tabTextes = (textEntryStruct *)malloc (NUM_MAX_TEXT_ENTRY *
                                           sizeof (textEntryStruct)); // 2000 = 250 * 8
    ...
    systemTextes =
        (u8 *)loadPakSafe (languageNameString, 0); // todo: use real language name
    ...
    while (currentPosInTextes < systemTextes + textLength)
    {
        ...
        if (currentIndex == '@') // start of string marker
        {
            ...
            while ((currentIndex = *(currentPosInTextes++)) >= '0' &&
                   currentIndex <= '9') // parse string number
            {
            ...
            if (currentIndex == ':') // start of string
            {
                ...
                {
                    currentPosInTextes++;
                } while ((unsigned char)*(currentPosInTextes - 1) >=
               ...
               int shadow)
{
    ...
    int textIndexMalloc =
        HQ_Malloc (HQ_Memory, getPakSize (languageNameString, index) + 300);
    ...
    while (!quit)
    {
            ...
            while (*ptrt == '#')
            {
                ...
                case 'G': // print number
                {
                    ...
                    if (loadPakToPtr ("ITD_RESS", 9, aux2))
                    {
                        ...
                        {
                        printTextSub5((((right-left)/2)+left)-var_C, currentTextY,
                        currentTextIdx, aux2); currentTextY = var_A;
                        }*/
                    }
            ...
            *(ptrt - 1) = 0; // add end of string marker to cut the word
            ...
            if (currentStringWidth <= maxStringWidth)
            {
                ...
                else
                {
                    ...
                    switch (
                        var_1C3) // eval the character that caused the 'end of word' state
                    {
            ...
            {
                renderText (
                    currentTextX, currentTextY, logicalScreen, currentText->textPtr);

                currentTextX +=
                    currentText->width + interWordSpace; // add inter word space
        ...
        pageChange:
            ...
            if (isFirstPage)
            {
                if (demoMode != 1)
                {
                    osystem_CopyBlockPhys (
                        (unsigned char *)logicalScreen, 0, 0, 320, 200);
                    FadeInPhys (16, 0);
                }
                else
                {
            ...
            else
            {
                if (turnPageFlag)
                {
                    if (previousPage < currentPage)
                    {
                        turnPageForward ();
                        osystem_CopyBlockPhys (
                            (unsigned char *)logicalScreen, 0, 0, 320, 200); // temp
                    }
                    else
                    {
                        turnPageBackward ();
...
void initEngine (void)
{
    ...
    if (g_gameId == AITD1)
    {
        listBody = HQR_InitRessource (listBodySelect[CVars[getCVarsIdx (CHOOSE_PERSO)]],
                                      37000,
                                      50); // was calculated from free mem size
        listAnim = HQR_InitRessource (listAnimSelect[CVars[getCVarsIdx (CHOOSE_PERSO)]],
                                      30000,
                                      80); // was calculated from free mem size
    }
    else
    {
        listBody = HQR_InitRessource (
            "LISTBODY", 37000, 50); // was calculated from free mem size
        listAnim = HQR_InitRessource (
            "LISTANIM", 30000, 80); // was calculated from free mem size
...
void loadMask (int cameraIdx)
{
    ...
    {
        cameraViewedRoomStruct *pRoomView =
            &cameraDataTable[currentCamera]->viewedRoomTable[i];
        ...
        for (int j = 0; j < pRoomView->numMask; j++)
        {
            unsigned char *pMaskData =
                pViewedRoomMask + READ_LE_U32 (pViewedRoomMask + j * 4);
            ...
            osystem_createMask (pDestMask->mask,
                                i,
                                j,
                                (unsigned char *)aux,
                                pDestMask->x1,
...
{
    for (int viewedRoomIdx = 0;
         viewedRoomIdx < cameraDataTable[currentCamera]->numViewedRooms;
         viewedRoomIdx++)
    {
        cameraViewedRoomStruct *pcameraViewedRoomData =
            &cameraDataTable[currentCamera]->viewedRoomTable[viewedRoomIdx];
        ...
        osystem_createMask(pDestMask->mask, i, j, (unsigned char*)aux, pDestMask->x1,
        pDestMask->y1, pDestMask->x2, pDestMask->y2);
        }*/
    }
...
void setupCameraSub1 ()
{
    ...
    *dataTabPos = -1;
    ...
    {
        if (!isInViewList (
                cameraDataTable[currentCamera]->viewedRoomTable[j].viewedRoomIdx))
        {
            *(dataTabPos++) =
                (char)cameraDataTable[currentCamera]->viewedRoomTable[j].viewedRoomIdx;
            *(dataTabPos) = -1;
        }
    }
}
...
void DeleteObjet (int index) // remove actor
{
    ...
    else
    {
        if (actorPtr->indexInWorld >= 0)
        {
            ...
            objectPtr->flags |=
                AF_SPECIAL * actorPtr->dynFlags; // ugly hack, need rewrite
...
int IsInCamera (int roomNumber)
{
    ...
    {
        if (cameraDataTable[currentCamera]->viewedRoomTable[i].viewedRoomIdx ==
            roomNumber)
        {
...
int updateActorAitd2Only (int actorIdx)
{
    ...
    if (currentActor->bodyNum != -1)
    {
        ...
        {
            if (IsInCamRectTestAITD2 (currentActor->roomX + currentActor->stepX,
                                      currentActor->roomZ + currentActor->stepZ))
            {
        ...
        else
        {
            if (currentObject->stage == g_currentFloor)
            {
                if (currentObject->life != -1)
                {
                    if (currentObject->lifeMode != -1)
                    {
                        ...
                        if (!di)
                        {
                            if (currentObject->body != -1)
                            {
                                ...
                                {
                                    if (IsInCamRectTestAITD2 (currentObject->x,
                                                              currentObject->z))
                                    {
                    ...
                    addObject:
                        actorIdx = copyObjectToActor (currentObject->body,
                                                      currentObject->typeZV,
                                                      currentObject->foundName,
                                                      ...
                                                      currentObject->anim,
                                                      currentObject->frame,
                                                      currentObject->animType,
                                                      currentObject->animInfo);
                        ...
                        if (actorIdx != -1)
                        {
                            ...
                            currentProcessedActorPtr->dynFlags =
                                (currentObject->flags & 0x20) / 0x20; // recheck
                            ...
                            setMoveMode (currentObject->trackMode,
                                         currentObject->trackNumber);

                            currentProcessedActorPtr->positionInTrack =
                                currentObject->positionInTrack;
...
void setupCamera ()
{
    ...
    if (debuggerVar_topCamera)
    {
        ...
        {
            x = objectTable[currentCameraTargetActor].worldX +
                objectTable[currentCameraTargetActor].stepX;
            y = debufferVar_topCameraZoom;
            z = objectTable[currentCameraTargetActor].worldZ +
                objectTable[currentCameraTargetActor].stepZ;
        }
    }
#endif
    ...
    setupCameraProjection (
        160, 100, pCamera->focal1, pCamera->focal2, pCamera->focal3); // setup focale
...
s16 updateActorRotation (interpolatedValue *rotatePtr)
{
    ...
    if (angleDif <= 0x200)
    {
        ...
        {
            s16 angle =
                ((rotatePtr->newAngle & 0x3FF) + 0x400) - ((rotatePtr->oldAngle & 0x3FF));
            return (((rotatePtr->oldAngle & 0x3FF)) +
                    ((angle * timeDif) / rotatePtr->param));
        }
    }
    else
    {
        int angle =
            (rotatePtr->newAngle & 0x3FF) - ((rotatePtr->oldAngle & 0x3FF) + 0x400);
...
void removeObjFromInventory (int objIdx)
{
    ...
    if (inventoryIdx != -1)
    {
        memmove (&inventoryTable[currentInventory][inventoryIdx],
...
void drawProjectedLine (s32 x1s, s32 y1s, s32 z1s, s32 x2s, s32 y2s, s32 z2s, int c)
{
    ...
    if (z1 > 0 && z2 > 0)
        osystem_draw3dLine (
            transformedX1, transformedY1, z1, transformedX2, transformedY2, z2, c);
#endif
}
...
void drawZv (tObject *actorPtr)
{
    ...
    {
        getZvRelativePosition (
            &localZv, actorPtr->room, objectTable[currentCameraTargetActor].room);
    }
    else
    {
    ...
    drawProjectedLine (localZv.ZVX1,
                       ...
                       localZv.ZVZ2,
                       10);
    drawProjectedLine (localZv.ZVX2,
                       localZv.ZVY2,
                       localZv.ZVZ2,
                       localZv.ZVX2,
                       localZv.ZVY2,
...
void drawConverZone (cameraZoneEntryStruct *zonePtr)
{
    ...
    {
        drawProjectedLine (zonePtr->pointTable[i].x * 10,
                           0,
                           zonePtr->pointTable[i].y * 10,
                           zonePtr->pointTable[i + 1].x * 10,
                           0,
                           zonePtr->pointTable[i + 1].y * 10,
                           20);
    }
    ...
    drawProjectedLine (zonePtr->pointTable[0].x * 10,
                       0,
                       zonePtr->pointTable[0].y * 10,
                       zonePtr->pointTable[i].x * 10,
                       0,
                       zonePtr->pointTable[i].y * 10,
                       20);
}
    ...
    for (i = 0; i < numCameraInRoom; i++)
    {
        ...
        for (j = 0; j < cameraDataTable[i]->numViewedRooms; j++)
        {
            ...
            if (cameraDataTable[i]->viewedRoomTable[j].viewedRoomIdx == currentRoom)
            {
                ...
                {
                    drawConverZone (
                        &cameraDataTable[i]->viewedRoomTable[j].coverZones[k]);
                }
            }
        }
    }
...
void drawMaskZone (cameraMaskStruct *maskZonePtr)
{
    ...
    {
        drawAAQuad (maskZonePtr->rectTests->zoneX1 * 10,
...
{
    {
        ...
        for (int j = 0; j < pCamera->numViewedRooms; j++)
        {
            ...
            // if(cameraDataTable[i]->viewedRoomTable[j].viewedRoomIdx ==
            // currentDisplayedRoom)
            {
...
void drawProjectedQuad (float x1,
                        float x2,
                        float x3,
                        float x4,
                        float y1,
                        ...
                        int transprency)
{
    ...
    if (z1 > DEPTH_THRESHOLD && z2 > DEPTH_THRESHOLD && z3 > DEPTH_THRESHOLD &&
        z4 > DEPTH_THRESHOLD)
    {
        osystem_draw3dQuad (transformedX1,
                            transformedY1,
                            z1,
                            transformedX2,
                            transformedY2,
...
void drawProjectedBox (
    int x1, int x2, int y1, int y2, int z1, int z2, int color, int transprency)
{
    // bottom
    drawProjectedQuad ((float)x1,
                       (float)x1,
                       (float)x2,
                       (float)x2,
                       (float)y1,
...
void drawRoomZv (ZVStruct *zoneData, int color, int transparency)
{
    ...
    drawProjectedBox (zoneData->ZVX1,
                      zoneData->ZVX2,
                      zoneData->ZVY1,
                      zoneData->ZVY2,
                      zoneData->ZVZ1,
...
void drawRoomZvLine (ZVStruct *zoneData, int color)
{
    ...
    drawProjectedLine (zoneData->ZVX1,
                       zoneData->ZVY1,
                       zoneData->ZVZ1,
                       zoneData->ZVX1,
                       zoneData->ZVY1,
    ...
    drawProjectedLine (zoneData->ZVX1,
                       ...
                       zoneData->ZVZ2,
                       color);
    drawProjectedLine (zoneData->ZVX2,
                       zoneData->ZVY1,
                       zoneData->ZVZ2,
                       zoneData->ZVX2,
                       zoneData->ZVY1,
...
void drawSceZone (int roomNumber)
{
    ...
    {
        memcpy (
            &dataLocal, &roomDataTable[roomNumber].sceZoneTable[i].zv, sizeof (ZVStruct));
...
void drawBgOverlay (tObject *actorPtr)
{
    ...
    if (g_gameId == AITD1)
    {
        ...
        for (int i = 0; i < numOverlayZone; i++)
        {
            ...
            if (isBgOverlayRequired (actorPtr->zv.ZVX1 / 10,
                                     actorPtr->zv.ZVX2 / 10,
                                     actorPtr->zv.ZVZ1 / 10,
                                     actorPtr->zv.ZVZ2 / 10,
                                     data + 4,
    ...
    for (i = 0; i < numActorInList; i++)
    {
        ...
        // if(actorPtr->_flags & (AF_ANIMATED + AF_DRAWABLE + AF_SPECIAL))
        {
            ...
            else
            {
                ...
                if (HQ_Load)
                {
                    //          initAnimInBody(actorPtr->FRAME, HQR_Get(listAnim,
                    //          actorPtr->ANIM), bodyPtr);
                }

                AffObjet (actorPtr->worldX + actorPtr->stepX,
                          actorPtr->worldY + actorPtr->stepY,
                          actorPtr->worldZ + actorPtr->stepZ,
                          actorPtr->alpha,
                          actorPtr->beta,
                          actorPtr->gamma,
                          bodyPtr);
            ...
            if (BBox3D1 <= 319 && BBox3D2 <= 199 && BBox3D3 >= 0 &&
                BBox3D4 >= 0) // is the character on screen ?
            {
                if (g_gameId == AITD1)
                {
...
{
    unsigned int Xdif =
        10 * (roomDataTable[destRoom].worldX - roomDataTable[startRoom].worldX);
    unsigned int Ydif =
        10 * (roomDataTable[destRoom].worldY - roomDataTable[startRoom].worldY);
    unsigned int Zdif =
        10 * (roomDataTable[destRoom].worldZ - roomDataTable[startRoom].worldZ);
...
void foundObject (int objIdx, int param)
{
    ...
    {
        if (timer - objPtr->trackNumber <
            300) // prevent from reopening the window every frame
            return;
    }
...
int checkForHardCol (ZVStruct *zvPtr, roomDataStruct *pRoomData)
{
    ...
    {
        if (((pCurrentEntry->zv.ZVX1) < (zvPtr->ZVX2)) &&
            ((zvPtr->ZVX1) < (pCurrentEntry->zv.ZVX2)))
        {
            if (((pCurrentEntry->zv.ZVY1) < (zvPtr->ZVY2)) &&
                ((zvPtr->ZVY1) < (pCurrentEntry->zv.ZVY2)))
            {
                if (((pCurrentEntry->zv.ZVZ1) < (zvPtr->ZVZ2)) &&
                    ((zvPtr->ZVZ1) < (pCurrentEntry->zv.ZVZ2)))
                {
...
int isInPoly (int x1, int x2, int z1, int z2, cameraViewedRoomStruct *pCameraZoneDef)
{
    ...
    for (i = 0; i < pCameraZoneDef->numCoverZones; i++)
    {
        ...
        for (j = 0; j < pCameraZoneDef->coverZones[i].numPoints; j++)
        {
            ...
            if (testCrossProduct (
                    xMid, zMid, xMid - 10000, zMid, zoneX1, zoneZ1, zoneX2, zoneZ2))
            {
                flag |= 1;
            }
...
int findBestCamera (void)
{
    ...
    for (i = 0; i < numCameraInRoom; i++)
    {
        ...
        if (currentCameraZoneList[i])
            if (isInPoly (
                    x1, x2, z1, z2, currentCameraZoneList[i])) // if in camera zone ?
            {
                // we try to select the best camera that looks behind the player
                int newAngle =
                    actorPtr->beta + (((cameraDataTable[i]->beta) + 0x200) & 0x3FF);
...
void checkIfCameraChangeIsRequired (void)
{
    ...
    if (currentCamera != -1)
    {
        ...
        if (isInPoly (zvx1,
                      ...
                      zvz1,
                      zvz2,
                      currentCameraZoneList[currentCamera])) // is still in current camera
                                                             // zone ?
        {
            return;
        }
        ...
        for (u32 i = 0; i < pRoomData->numSceZone; i++)
        {
                    ...
                    &pCurrentZone->zv))
            {
                ...
                case 0:
                {
                    ...
                    x = (roomDataTable[currentProcessedActorPtr->room].worldX -
                         roomDataTable[oldRoom].worldX) *
                        10;
                    y = (roomDataTable[currentProcessedActorPtr->room].worldY -
                         roomDataTable[oldRoom].worldY) *
                        10;
                    z = (roomDataTable[currentProcessedActorPtr->room].worldZ -
                         roomDataTable[oldRoom].worldZ) *
                        10;
                ...
                case 8:
                {
                    ...
                    if (g_gameId != AITD1)
                    {
                        currentProcessedActorPtr->hardMat =
                            (short)pCurrentZone->parameter;
                    }
                    break;
                }
                case 9: // Scenar
                {
                    ...
                    {
                        currentProcessedActorPtr->HARD_DEC =
                            (short)pCurrentZone->parameter;
                    }
                    break;
                }
...
int checkLineProjectionWithActors (
    int actorIdx, int X, int Y, int Z, int beta, int room, int param)
{
    ZVStruct localZv;
    int foundFlag = -2;
    int tempX;
    ...
    while (foundFlag == -2)
    {
        ...
        else
        {
            ...
            for (i = 0; i < NUM_MAX_OBJECT; i++)
            {
                if (currentActorPtr->indexInWorld != -1 && i != actorIdx &&
...
void throwStoppedAt (int x, int z)
{
    ...
    while (!foundPosition)
    {
        ...
        if (foundPosition)
        {
            if (y2 < -500)
            {
                zvCopy.ZVY1 += 100; // is the object reachable ? (100 is Carnby height. If
                                    // hard col at Y + 100, carnby can't reach that spot)
                zvCopy.ZVY2 += 100;

                if (!checkForHardCol (&zvCopy,
        ...
        for (i = 0; i < 5; i++)
        {
            if (currentMessage->string)
            {
                ...
                else
                {
                    ...
                    renderText (
                        X, var_10 + 1, logicalScreen, currentMessage->string->textPtr);
                }
